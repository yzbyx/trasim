...
void Road::step_lane_change(LaneAbstract *lane) {
...
std::tuple<LaneAbstract *, LaneAbstract *, std::vector<SECTION_TYPE>>
Road::get_available_adjacent_lane(LaneAbstract *lane, double pos, VType car_type) {
...
void Road::lc_take_over(int car_id, const std::tuple<LaneAbstract *, double, double, double> &lc_result) {
    for (auto lane: this->lane_list) {
        for (auto car: lane->car_list) {
            if (car->ID == car_id) {
...
//                std::cout << ALL_C_INFO_2_STRING.at(name) << ": " <<
//                lane->data_container->lane_total_data[name].size() << std::endl;
